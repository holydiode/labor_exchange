...
from schemas import LoginSchema, PairTokenSchema
...
async def login(login_shcema: LoginSchema, db: AsyncSession = Depends(get_db)):
    user = await user_queries.get_by_email(db=db, email=login_shcema.email)

    if user is None or not verify_password(login_shcema.password, user.hashed_password):
...
async def get_pair_token_by_refresh(current_user: User = Depends(get_refreshed_user)):
    return PairTokenSchema(
